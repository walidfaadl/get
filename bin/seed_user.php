 ...
 *
 *   php bin/seed_user.php <role> <name> <username> <password> [department] [email]
 *
 * role: manager | head
 * إن كان اسم المستخدم موجوداً يُحدَّث، وإلا يُنشأ.
 * تُنشئ الجداول وتُطبّق الترحيلات تلقائياً إن لزم.
 */
...
$argvv = $argv ?? [];
if (count($argvv) < 5) {
    fwrite(STDERR, "الاستعمال: php bin/seed_user.php <role> <name> <username> <password> [department] [email]\n");
    exit(2);
}
